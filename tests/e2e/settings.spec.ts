...
test.describe( 'Settings page', () => {
	...
	test( 'saves the guide text', async ( { page } ) => {
		...
		await textarea.fill(
			'No synergy. No leverage. Spell out numbers below ten.'
		);
	...
	test( 'renders the upload nonce field', async ( { page } ) => {
		...
		await expect(
			page.locator( 'input[name="sgr_upload_guide_nonce"]' )
		).toHaveCount( 1 );
	} );

	test( 'uploads a guide file and persists its contents', async ( {
		page,
	} ) => {
		await page.goto( '/wp-admin/options-general.php?page=sgr-settings' );

		await page.setInputFiles( '#sgr_guide_file', {
			name: 'brand-guide.md',
			mimeType: 'text/markdown',
			buffer: Buffer.from(
				'# Voice\n\nAvoid synergy. Prefer plain English.'
			),
		} );
		...
		await expect( page.locator( '#sgr_guide_text' ) ).toHaveValue(
			/# Voice[\s\S]*Prefer plain English\./
		);
	} );
} );
